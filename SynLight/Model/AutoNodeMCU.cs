...
namespace SynLight.Model
{
    public class AutoNodeMCU : ModelBase
    {
        ...
        public AutoNodeMCU()
        {
            ...
            //FindNodeMCU(); //Called elsewhere in Process_SynLight
        }
        public void FindNodeMCU()
        {
            if (!staticConnected)
            {
                ...
                for (byte n = 1; n < 254; n++) //Checking local IP from 1 to 254
                {
                    ...
                    //For some reason 137 is used on my setup when using mobile hotstop ...
                    endPoint = new IPEndPoint(new IPAddress(new byte[4] { 192, 168, 137, n }), UDPPort); //Try with 192.168.137.N
                    try { SendPayload(PayloadType.ping, new List<byte>(ping)); }
                    catch { }
                    System.Threading.Thread.Sleep(10);
